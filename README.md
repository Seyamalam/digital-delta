...
- React, TypeScript, Vite, a bundled SVG topology map, and locally packaged fonts for the command dashboard
...
M8 is now demoable on that same emulator. The bundled graph includes air-only N7, the field engine classifies it as drone-required, evaluates three rendezvous candidates, chooses R3 by delivery-completion time, and rejects plans below the 20 percent battery reserve. Its bilingual animated flow persists rendezvous and boat/drone state as Protobuf events, creates a real RSA-PSS boat-to-simulated-drone QR offer, and appends the two-party receipt to the local custody chain. All 37 connected tests pass, including the packaged graph, Room event parsing, custom Android Keystore custody, and the bilingual Compose journey. Paired ready and transferred captures are in `artifacts/screenshots/`.

The new Delta Command projector app is also runnable offline. It starts in Bangla, switches to an information-equivalent English view without resetting the exercise, fits the complete operations surface at 1366 by 768 and 1920 by 1080, and advances a deterministic six-step scenario through prediction, road failure, CRDT conflict, low-battery mesh throttling, and verified custody. Its fault lab adds explicit observer syncing, relay-node loss with queue retention, simulated vehicle delay, duplicate rejection, and signature-tamper rejection. The scenario can advance manually or replay and pause automatically. Its topology map, inventory, node, queue, event-ledger, and local Disaster Control panels use no remote tiles, APIs, or fonts. Seven browser-domain tests and the production web build now run inside `scripts/verify-local.sh`. The observer transport is intentionally still pending; the current command experience is a seeded presentation surface, not a live source of field truth.

The minified ARM64 release has also run the model and language switch on an Android 16 emulator at a measured peak of 67,504 KB PSS across three post-inference readings. These are integrated foundations, not a claim that all eight modules are complete. Delayed-boat replanning, camera scanning for provisioning and handoff codes, cross-phone credential binding, signed application-layer peer identity and acknowledgements, the physical three-phone relay, physical-phone memory evidence, MapLibre offline regions, the command observer transport, and several Disaster Control injections remain under active implementation. Current evidence is recorded in `artifacts/` and can be reproduced with `scripts/verify-local.sh --connected`.
