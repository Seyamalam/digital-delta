...
One APK supports three allow-listed fair profiles: N4 clinic, N6 hospital, and RLY-01 relay. The selected profile is stored in local Protobuf DataStore and determines the Keystore aliases, enrollment request, Nearby endpoint name, ingress node, and acknowledgement signer. Changing it stops the foreground relay first. A new profile cannot authenticate until the administrator issues its new enrollment credential and that phone installs it. Free-form node IDs are deliberately not accepted from the UI.
