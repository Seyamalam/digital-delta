...
**Decision:** Feed the selected M4 route ETA into `triage-v2`, evaluate the required 30-percent slowdown case, and treat arrival exactly at an SLA as protected. Reject estimates older than five minutes both when proposing and when confirming. Rank simultaneous urgent cargo by priority tier, remaining SLA, and stable cargo ID without treating another P0/P1 as depositable. If P0 or P1 would arrive late, propose the safest deterministic lower-priority drop but do not apply it without a coordinator confirmation. Record one Protobuf decision event before displaying success.
...
**Consequences:** The current seeded mission moves from protected to a P0 breach proposal when simulated `E3` failure changes its ETA from 65 to 200 minutes. A second simulated P0 remains in the urgent queue and is never silently dropped. The event records the policy version, reason code, coordinator, both affected cargo IDs, waypoint, and estimated time gain. Confirmation is single-flight to prevent rapid duplicate taps and fails closed if the route data ages out. The event and vector-clocked cargo assignment commit atomically, with a deterministic convergence hash; this is an operational assignment decision, not proof that a physical deposit occurred. A signed envelope for the preemption event itself remains hardening work.
...
## DD-017: minimize delivery completion while preserving drone reserve

**Status:** Accepted

**Decision:** Classify reachability with the same vehicle-constrained graph as M4. For an air-only destination, reject any rendezvous whose complete drone mission—base to handoff to destination—would finish below the configured reserve. Among feasible candidates, minimize final delivery arrival, then handoff readiness, then candidate ID. Persist the selected plan and each simulated arrival as Protobuf events, and reuse M5 unchanged for the signed boat-to-drone custody record.

**Reason:** Minimizing only the later arrival at the handoff can select a point that delays the actual clinic delivery or strands the simulated drone. A reserve-first feasibility boundary is safer and makes the choice explainable from the three displayed ETAs.
